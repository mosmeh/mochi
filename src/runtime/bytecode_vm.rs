    ...
    'start: loop {
        ...
        while let Some(&insn) = code.get(pc) {
            ...
            match insn.raw_opcode() {
                ...
                opcode::IDIV => ops::do_arithmetic(stack, &mut pc, insn, ops::idivi, ops::idivf),
                ...
                opcode::LEN => {
                    ...
                    let len = match rb {
                        ...
                        Value::Table(table) => {
                            if vm
                                .borrow(gc)
                                .metamethod_of_object(gc, Metamethod::Len, rb)
                                .is_some()
                            {
                                thread_ref.save_pc(pc);
                                todo!();
                ...
                opcode::CONCAT => {
                    ...
                    if b >= 1 {
                        ...
                        for (i, value) in stack[a..].iter().take(b).enumerate().rev() {
                            ...
                            thread_ref.save_pc(pc);
                            /*super::metamethod::concat_slow_path(
                                gc,
                                &mut thread_ref,
                                lhs_index,
                                rhs,
                ...
                opcode::EQ => {
                    ...
                    if ra == rb {
                        ...
                        .borrow(gc)
                        .metamethod_of_object(gc, Metamethod::Eq, ra)
                        .is_some()
                        || vm
                            .borrow(gc)
                            .metamethod_of_object(gc, Metamethod::Eq, rb)
                            .is_some()
                    {
                        thread_ref.save_pc(pc);
                        todo!();
                ...
                opcode::CALL => {
                    ...
                    drop(thread_ref);
                    to_rooted!(roots, thread);
                    super::call_value(gc, roots, vm, *thread, base + a)?;
                    continue 'start;
                }
                opcode::TAILCALL => {
                    let a = insn.a();
                    ...
                    drop(thread_ref);
                    to_rooted!(roots, thread);
                    super::call_value(gc, roots, vm, *thread, bottom)?;
                    continue 'start;
                }
                opcode::RETURN => {
                    if insn.k() {
                ...
                opcode::TFORCALL => {
                    ...
                    drop(thread_ref);
                    to_rooted!(roots, thread);
                    super::call_value(gc, roots, vm, *thread, new_bottom)?;
                    continue 'start;
                }
                opcode::TFORLOOP => {
                    let a = insn.a();
