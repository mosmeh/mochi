...
pub fn create_table(heap: &GcHeap) -> Table {
    ...
    table.set(
        heap.allocate(LuaString::from("char")),
    ...
    table.set(
        heap.allocate(LuaString::from("len")),
    ...
    table.set(
        heap.allocate(LuaString::from("lower")),
        heap.allocate(NativeClosure::new(|heap, vm, key| {
            let string = get_string_arg(vm, key.clone(), 1)?;
    ...
    table.set(
        heap.allocate(LuaString::from("reverse")),
        heap.allocate(NativeClosure::new(|heap, vm, key| {
    ...
    table.set(
        heap.allocate(LuaString::from("upper")),
        heap.allocate(NativeClosure::new(|heap, vm, key| {
            let string = get_string_arg(vm, key.clone(), 1)?;
