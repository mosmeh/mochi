...
pub struct Bucket<'gc> {
    key_tag: Tag,
    key_payload: Payload<'gc>,
    value_tag: Tag,
...
impl<'gc> Bucket<'gc> {
    ...
    pub fn matches(&self, key: Value<'gc>) -> bool {
    ...
    pub fn set_new_item(&mut self, key: Value<'gc>, value: Value<'gc>) {
...
impl<'gc> Value<'gc> {
    unsafe fn from_parts(tag: Tag, payload: Payload<'gc>) -> Self {
        match tag {
            Tag::Nil => Self::Nil,
    ...
    unsafe fn eq_parts(&self, tag: Tag, payload: Payload<'gc>) -> bool {
        match self {
    ...
    fn into_parts(self) -> (Tag, Payload<'gc>) {
        match self {
